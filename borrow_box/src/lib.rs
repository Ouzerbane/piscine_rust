...
impl GameSession {
    ...
    pub fn update_score(&mut self, user_name: &str) {
        ...
        if &self.p1.0 == user_name && &self.p2.1 <= &(self.nb_games/2)&& &self.p1.1 <= &(self.nb_games/2){
            self.p1.1 += 1
        }else if &self.p2.0 == user_name && &self.p1.1 <= &(self.nb_games/2)&& &self.p2.1 <= &(self.nb_games/2){
            self.p2.1 += 1
        }else {
            return ;
        }
